...
export default function CoursePage() {
    ...
    const handleAddNote = (textParam) => {
        const textToSave = (typeof textParam === 'string' ? textParam : newNote).trim();
    ...
    return (
        <div className="course-layout">
                                ...
                                <section id={activeTopic.id} key={activeTopic.id} className="topic-section" style={{ marginBottom: '4rem' }}>
                                    ...
                                    <div className="topic-notes-appended" style={{ marginTop: '4rem', paddingTop: '2.5rem', borderTop: '1px solid var(--border)' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.75rem' }}>
                                            <h3 style={{ fontSize: '1.5rem', fontWeight: '600', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '0.5rem', margin: 0 }}>
                                        ...
                                        {notes.length === 0 ? (
                                            <div style={{ textAlign: 'center', padding: '2rem 1rem', background: 'rgba(255, 255, 255, 0.02)', border: '1px dashed var(--border)', borderRadius: 'var(--radius-md)' }}>
                                                <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', margin: 0 }}>
                                                    No notes yet for this topic. Use the box above or the floating button to add your first note!
                                                </p>
                                            </div>
                                        ) : (
            ...
            <button className="btn btn-primary global-add-notes-btn" onClick={() => setIsNotesOpen(true)}>
            ...
            {/* Notes Panel / Modal */}
            <div className={`notes-panel-overlay ${isNotesOpen ? 'show' : ''}`} onClick={() => setIsNotesOpen(false)}></div>
            <div className={`notes-panel ${isNotesOpen ? 'open' : ''}`}>
                <div className="notes-panel-header">
                    <h3>My Notes</h3>
                    <button className="btn-close-notes" onClick={() => setIsNotesOpen(false)}>&times;</button>
                </div>
                <div className="notes-list">
                    {notes.length === 0 ? (
                        <p className="no-notes">No notes yet for this topic.</p>
                    ) : (
                        notes.map(note => {
                            const isEditing = editingNoteId === note.id;
                            return (
                                <div key={note.id} className="note-item" style={{ position: 'relative' }}>
                                    <div className="note-card-actions" style={{ position: 'absolute', top: '0.6rem', right: '0.6rem' }}>
                                        {!isEditing && (
                                            <button 
                                                type="button" 
                                                className="note-action-btn note-edit-btn" 
                                    ...
                                    {isEditing ? (
                                        ...
                                        <>
                                            <p style={{ paddingRight: '3.5rem' }}>{note.text}</p>
                ...
                <div className="notes-input-area">
                    <textarea 
                        ...
                        className="note-textarea"
                    ></textarea>
                    <button className="btn btn-primary" style={{width: '100%', justifyContent: 'center'}} onClick={() => handleAddNote(newNote)}>Save Note</button>
                </div>
            </div>
        </div>
    );
